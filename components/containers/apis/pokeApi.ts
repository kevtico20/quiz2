...
export async function obtenerPokemon(nombrePokemon: string): Promise<Pokemon> {
    try {
        ...
        const existingPokemonIndex = pokemonDB.findIndex(pokemon => pokemon.nombre === data.name);
...
export async function obtenerTodosLosPokemon(): Promise<Pokemon[]> {
    try {
        ...
        const pokemons = await Promise.all(data.results.map(async (pokemon: { name: string }) => {
